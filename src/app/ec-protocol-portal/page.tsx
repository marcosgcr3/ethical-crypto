...
export default async function AdminDashboard() {
  ...
  const renderArticleList = (list: typeof articles, emptyMessage: string) => (
    <div className="bg-[#0A0D11] rounded-2xl shadow-[0_4px_40px_rgba(0,0,0,0.3)] border border-white/5 overflow-hidden mb-12">
      <ul className="divide-y divide-white/5">
        <li className="grid grid-cols-12 gap-4 px-8 py-5 bg-[#05070A] text-[10px] font-black uppercase tracking-[0.2em] text-white">
        ...
        {list.length === 0 ? (
          ...
          list.map((article) => (
              <li key={article.id} className="grid grid-cols-12 gap-4 px-8 py-6 items-center hover:bg-white/[0.02] transition-colors border-l-2 border-transparent hover:border-white">
                <div className="col-span-6 md:col-span-3 font-bold text-sm text-white line-clamp-1" title={article.title}>
                  {article.title}
                </div>
                <div className="col-span-4 md:col-span-2">
                  <span className="text-[9px] font-black uppercase tracking-widest text-white bg-white/10 px-2 py-1 rounded-full border border-white/20">
                      {article.category}
                  </span>
                </div>
  ...
  return (
    <div className="min-h-screen bg-[#05070A] text-white selection:bg-white/30">
      <div className="container mx-auto px-8 py-24 pb-48">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-16 border-b border-white/5 pb-12">
          <div>
            ...
            <p className="text-white/40 text-sm font-bold uppercase tracking-[0.1em]">Sovereign Protocol Management System <span className="text-white/40">v4.0.2</span></p>
          </div>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-4 mb-8">
            <div className="w-3 h-3 rounded-full bg-white shadow-[0_0_15px_rgba(255,255,255,0.2)]"></div>
