...
export default async function FundamentalsPillarPage() {
  let articles: any[] = [];
  try {
    articles = await prisma.article.findMany({
      where: { category: 'fundamentals', published: true },
      orderBy: { createdAt: 'desc' },
    });
  } catch (error) {
    console.error("Database fetch failed for Fundamentals pillar page.", error);
  }

  const faqItems = [
    {
      question: 'What is "Tokenomics Architecture"?',
      answer: 'Tokenomics Architecture is the economic engineering behind a protocol. It defines the supply-demand balance, inflation schedules, and value accrual mechanisms that determine a network\'s long-term viability.'
    },
    {
      question: 'Why do network fundamentals matter?',
      answer: 'Fundamentals provide the "ground truth" of a protocol. By analyzing on-chain metrics like developer activity and TVL growth, we can separate technical utility from speculative noise.'
    }
  ];
              ...
              {articles.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  {articles.map((article) => (
                    <Link key={article.id} href={`/fundamentals/${article.slug}`} className="group relative block cursor-pointer">
                      <article>
                        {article.imageUrl ? (
                          <div className="relative overflow-hidden rounded-3xl mb-6 bg-zinc-50 h-56 border border-zinc-100 shadow-sm transition-all group-hover:border-black/10 group-hover:shadow-md">
                            ...
                            <div className="absolute top-6 left-6">
                              <span className="bg-white/90 backdrop-blur-md px-4 py-2 rounded-xl text-[10px] font-black uppercase text-black tracking-widest border border-black/5 shadow-sm">FUNDAMENTALS</span>
                            </div>
                          </div>
                        ) : (
                          <div className="mb-4">
                             <span className="bg-black text-white px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest">FUNDAMENTALS</span>
                          </div>
                        )}
                ...
                <div className="bg-zinc-50 rounded-[2.5rem] border border-zinc-100 p-16 text-center shadow-inner">
                   ...
                   <h3 className="font-heading text-2xl font-black text-black mb-4 uppercase tracking-tight">No fundamental reports found</h3>
                   <p className="text-zinc-500 font-medium max-w-md mx-auto leading-relaxed">
                      Our network intelligence unit is currently auditing protocol architectures. Verified code logic reports will be deployed shortly.
                   </p>
                </div>
              )}
            </section>
