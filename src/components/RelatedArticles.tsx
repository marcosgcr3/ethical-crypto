  ...
  return (
    <section className="py-16 mt-16 border-t border-black/5">
      <div className="flex items-center gap-4 mb-8">
        <h3 className="font-heading text-3xl font-black text-black uppercase tracking-tight">
          Next <span className="text-zinc-300">Logic Node</span>
        </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {articles.map((article) => (
            ...
            >
              {/* Image Container */}
              {article.imageUrl ? (
                <div className="relative aspect-[16/10] overflow-hidden bg-black/5 rounded-[1.5rem] mb-4">
                  <Image 
                ...
                <div className="mb-4">
                  <span className="bg-black text-white px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest">{article.category}</span>
                </div>
              )}

              {/* Content Container */}
              <div className="p-4 flex flex-col flex-grow">
                <h4 className="font-heading text-base font-extrabold text-black group-hover:text-zinc-500 transition-colors line-clamp-2 leading-tight mb-3">
                  {article.title}
                </h4>
                ...
                <div className="pt-4 border-t border-black/5 flex items-center justify-between text-[10px] font-black text-zinc-300 mt-auto">
                  <span className="uppercase tracking-widest">
                    {new Date(article.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                  </span>
                  <span className="group-hover:text-black transition-colors flex items-center gap-2 uppercase tracking-[0.2em]">
